...
isoform_counts_1 <- readRDS("isoform_counts_1.rds")
```
...
```{r}
stats_deseq2_1 <- -log10(joint_clean_1$pvalue_deseq2)
...
names(stats_joint_1) <- joint_clean_1$gene_id

```

```{r}
fg_deseq2_j_1 <- fgseaMultilevel(
  pathways = BP_list,
  stats = sort(stats_deseq2_1, decreasing = TRUE),
  scoreType = "pos"
)

fg_dexseq_j_1 <- fgseaMultilevel(
  pathways = BP_list,
  stats = sort(stats_dexseq_1, decreasing = TRUE),
  scoreType = "pos"
)

fg_joint_j_1 <- fgseaMultilevel(
  pathways = BP_list,
...
sig_sets_joint_1  <- sig_paths(fg_joint_j_1)
...
fg_deseq2_mf_1 <- fgseaMultilevel(
  pathways = MF_list,
  stats = sort(stats_deseq2_1, decreasing = TRUE),
  scoreType = "pos"
)

fg_dexseq_mf_1 <- fgseaMultilevel(
  pathways = MF_list,
  stats = sort(stats_dexseq_1, decreasing = TRUE),
  scoreType = "pos"
)

fg_joint_mf_1 <- fgseaMultilevel(
  pathways = MF_list,
  stats = sort(stats_joint_1, decreasing = TRUE),
  scoreType = "pos"
)
...
```{r}
sig_sets_deseq2_mf_1 <- sig_paths(fg_deseq2_mf_1)
sig_sets_dexseq_mf_1 <- sig_paths(fg_dexseq_mf_1)
sig_sets_joint_mf_1 <- sig_paths(fg_joint_mf_1)

c(DESeq2 = length(sig_sets_deseq2_mf_1),
  DEXSeq = length(sig_sets_dexseq_mf_1),
  Joint  = length(sig_sets_joint_mf_1))
```
